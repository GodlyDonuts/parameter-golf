...
_POLAR_EXPRESS_NS=bool(int(os.environ.get('POLAR_EXPRESS_NS','0')))
@torch.compile
def zeropower_via_newtonschulz5(G,steps=10,eps=1e-07):
	...
	if _POLAR_EXPRESS_NS:
		...
		for(a,b,c)in coeffs:A=X@X.T;B=b*A+c*A@A;X=a*X+B@X
	else:
...
def _nm_install_hooks(model,capture_every=1):
	'''Install forward-pre hooks on every CastedLinear so we can accumulate Z^T@Z (the right-Gram of inputs) per layer for Newton-Muon preconditioning. Idempotent: clears any prior installation.'''
	for h in _NM_HANDLES:h.remove()
	_NM_HANDLES.clear();_NM_PARAM_TO_MODULE.clear();_NM_STEP_COUNTER[0]=0
	def make_hook(m):
		def hook(module,args):
			if _NM_STEP_COUNTER[0]%capture_every!=0:return
			x=args[0];x_flat=x.reshape(-1,x.size(-1)).float();outer=x_flat.transpose(0,1)@x_flat;n=x_flat.size(0)
			if getattr(module,'_nm_K_acc',None)is None or module._nm_K_acc.shape!=outer.shape:module._nm_K_acc=outer.detach();module._nm_K_count=n
			else:module._nm_K_acc.add_(outer.detach());module._nm_K_count+=n
		return hook
	count=0
	for(name,mod)in model.named_modules():
		if isinstance(mod,CastedLinear)and mod.weight.ndim==2:
			mod._nm_K_acc=None;mod._nm_K_count=0;_NM_PARAM_TO_MODULE[id(mod.weight)]=mod;_NM_HANDLES.append(mod.register_forward_pre_hook(make_hook(mod)));count+=1
	log(f'newton_muon: installed {count} forward-pre hooks (capture_every={capture_every})')
def _nm_uninstall_hooks():
	for h in _NM_HANDLES:h.remove()
	_NM_HANDLES.clear();_NM_PARAM_TO_MODULE.clear()
def _nm_advance_step():_NM_STEP_COUNTER[0]+=1
class Muon(torch.optim.Optimizer):
	def __init__(self,params,lr,momentum,backend_steps,nesterov=True,weight_decay=.0,row_normalize=False,newton_muon=False,nm_beta=.95,nm_gamma=.2,nm_k=32,nm_warmup=50,nm_all_reduce_k=False):super().__init__(params,dict(lr=lr,momentum=momentum,backend_steps=backend_steps,nesterov=nesterov,weight_decay=weight_decay,row_normalize=row_normalize,newton_muon=newton_muon,nm_beta=nm_beta,nm_gamma=nm_gamma,nm_k=nm_k,nm_warmup=nm_warmup,nm_all_reduce_k=nm_all_reduce_k))
	def _nm_apply(self,p,g,group):
		'''Newton-Muon right-preconditioning: g <- g @ (K + gamma*tr(K)/n * I)^-1 where K is the EMA of Z^T@Z.'''
		mod=_NM_PARAM_TO_MODULE.get(id(p))
		if mod is None or getattr(mod,'_nm_K_acc',None)is None or mod._nm_K_count<=0:return g
		state=self.state[p];beta=group['nm_beta'];nm_k=group['nm_k'];nm_warmup=group['nm_warmup'];nm_gamma=group['nm_gamma'];all_red=group['nm_all_reduce_k']
		k_inst=mod._nm_K_acc/mod._nm_K_count
		if all_red and dist.is_available()and dist.is_initialized():dist.all_reduce(k_inst,op=dist.ReduceOp.AVG)
		if'nm_K'not in state:state['nm_K']=k_inst.detach().clone();state['nm_K_inv']=None;state['nm_step']=0
		else:state['nm_K'].mul_(beta).add_(k_inst,alpha=1.-beta)
		state['nm_step']+=1;mod._nm_K_acc.zero_();mod._nm_K_count=0
		if state['nm_step']>=nm_warmup and(state['nm_K_inv']is None or state['nm_step']%nm_k==0):
			K=state['nm_K'];n=K.size(0);gamma_eff=nm_gamma*torch.diagonal(K).sum().item()/max(n,1);state['nm_K_inv']=torch.linalg.inv(K+gamma_eff*torch.eye(n,device=K.device,dtype=K.dtype))
		if state['nm_K_inv']is not None and state['nm_step']>=nm_warmup:g=g@state['nm_K_inv'].to(g.dtype)
		return g
	@torch.no_grad()
	def step(self,closure=None):
		...
		for group in self.param_groups:
			...
			lr=group['lr'];momentum=group['momentum'];backend_steps=group['backend_steps'];nesterov=group['nesterov'];nm_on=group.get('newton_muon',False);total_params=sum(int(p.numel())for p in params);updates_flat=torch.zeros(total_params,device=params[0].device,dtype=torch.bfloat16);curr=0
			for(i,p)in enumerate(params):
				if i%world_size==rank and p.grad is not None:
					g=p.grad
					if nm_on:g=self._nm_apply(p,g,group)
					state=self.state[p]
...
class Optimizers:
	def __init__(self,h,base_model):
		...
		token_lr=h.tied_embed_lr if h.tie_embeddings else h.embed_lr;tok_params=[{'params':[base_model.tok_emb.weight],'lr':token_lr,'base_lr':token_lr}];self.optimizer_tok=torch.optim.AdamW(tok_params,betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.embed_wd,fused=True);self.optimizer_muon=Muon(matrix_params,lr=h.matrix_lr,momentum=h.muon_momentum,backend_steps=h.muon_backend_steps,weight_decay=h.muon_wd,row_normalize=h.muon_row_normalize,newton_muon=h.newton_muon_enabled,nm_beta=h.newton_muon_beta,nm_gamma=h.newton_muon_gamma,nm_k=h.newton_muon_k_refresh,nm_warmup=h.newton_muon_warmup,nm_all_reduce_k=h.newton_muon_all_reduce_k)
...
def _lqer_pack(A,B,bits):
	rng=2**(bits-1)-1;sA=(A.abs().amax(dim=1).clamp_min(1e-10)/rng).to(torch.float16);sB=(B.abs().amax(dim=1).clamp_min(1e-10)/rng).to(torch.float16);qA=torch.clamp(torch.round(A/sA.float().view(-1,1)),-rng,rng).to(torch.int8);qB=torch.clamp(torch.round(B/sB.float().view(-1,1)),-rng,rng).to(torch.int8)
	return qA,sA,qB,sB
def _lqer_pack_asym(A,B,g=64):
	sA=(A.abs().amax().clamp_min(1e-10)/1.5).to(torch.float16);qA=torch.clamp(torch.round(A/sA.float()),-2,1).to(torch.int8);Bf=B.reshape(-1,g);Bmax=Bf.abs().amax(dim=-1,keepdim=True).clamp_min(1e-10);sB=(Bmax/7.5).to(torch.float16).reshape(-1);qB=torch.clamp(torch.round(Bf/sB.float().reshape(-1,1)),-8,7).to(torch.int8).reshape(B.shape)
	return qA,sA,qB,sB
def gptq_mixed_quantize(state_dict,hessians,h):
	result={};meta={};lqer_on=bool(getattr(h,'lqer_enabled',False));lqer_cands={}
	for(name,tensor)in state_dict.items():
		...
		if lqer_on:W_q=q.float()*s.float().view(-1,1);E=t.float()-W_q;lqer_cands[name]=(E,float(E.norm()))
	if lqer_on and lqer_cands:
		top=sorted(lqer_cands.items(),key=lambda kv:-kv[1][1])[:h.lqer_top_k];asym_on=bool(getattr(h,'lqer_asym_enabled',False));asym_g=int(getattr(h,'lqer_asym_group',64))
		for(name,(E,_norm))in top:
			U,S,Vh=torch.linalg.svd(E,full_matrices=False);r=min(h.lqer_rank,S.numel());A=(U[:,:r]*S[:r]).contiguous();B=Vh[:r,:].contiguous()
			if asym_on and B.numel()%asym_g==0:
				qA,sA,qB,sB=_lqer_pack_asym(A,B,asym_g);result[name+'.lqA_a']=qA;result[name+'.lqAs_a']=sA;result[name+'.lqB_a']=qB;result[name+'.lqBs_a']=sB;meta[name]=meta[name]+'+lqer_asym'
			else:
				qA,sA,qB,sB=_lqer_pack(A,B,h.lqer_factor_bits);result[name+'.lqA']=qA;result[name+'.lqAs']=sA;result[name+'.lqB']=qB;result[name+'.lqBs']=sB;meta[name]=meta[name]+'+lqer'
...
def dequantize_mixed(result,meta,template_sd):
	...
	for(name,orig)in template_sd.items():
		...
		if'lqer_asym'in info:
			qA_t=result[name+'.lqA_a'];sA_t=result[name+'.lqAs_a'];qB_t=result[name+'.lqB_a'];sB_t=result[name+'.lqBs_a'];qA=qA_t.float()*float(sA_t);g_sz=qB_t.numel()//sB_t.numel();qB=(qB_t.reshape(-1,g_sz).float()*sB_t.float().view(-1,1)).reshape(qB_t.shape);W=W+qA@qB
		elif'lqer'in info:
			qA=result[name+'.lqA'].float()*result[name+'.lqAs'].float().view(-1,1);qB=result[name+'.lqB'].float()*result[name+'.lqBs'].float().view(-1,1);W=W+qA@qB
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:filter={h.ttt_param_filter} num_params={len(ttt_params)} num_floats={sum(p.numel() for p in ttt_params)} wd={h.ttt_wd} clip={h.ttt_grad_clip} mom_reset={h.ttt_momentum_reset} sched={h.ttt_schedule} lr_offset={h.ttt_lr_offset}");optimizer=torch.optim.SGD(ttt_params,lr=h.ttt_lr,momentum=h.ttt_momentum,weight_decay=h.ttt_wd)
	for ci in range(num_chunks):
		...
		if not is_last_chunk and h.ttt_epochs>0:
			...
			if chunk_seqs>0:
				_co=h.ttt_lr_offset;_de=max(num_chunks-1-_co,1);_ci=max(ci-_co,0)
				if h.ttt_schedule=='constant':cos_lr=h.ttt_lr
				elif h.ttt_schedule=='linear':cos_lr=h.ttt_lr*max(1.-_ci/_de,0.)
				else:cos_lr=h.ttt_lr*.5*(1.+math.cos(math.pi*_ci/_de))
				...
				if h.ttt_momentum_reset:optimizer.state=collections.defaultdict(dict)
	if dist.is_available()and dist.is_initialized():dist.all_reduce(loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(token_count,op=dist.ReduceOp.SUM);dist.all_reduce(byte_count,op=dist.ReduceOp.SUM)
...
def train_model(h,device,val_data):
	...
	if h.newton_muon_enabled:_nm_install_hooks(base_model,capture_every=h.newton_muon_capture_every);compiled_model=torch.compile(base_model,dynamic=False,fullgraph=False);log(f'newton_muon: compile fullgraph=False (hooks active)')
	...
	def step_fn(step,lr_scale):
		...
		optimizers.step()
		if h.newton_muon_enabled:_nm_advance_step()
		return train_loss
	if h.warmup_steps>0:
	...
	if h.newton_muon_enabled:_nm_uninstall_hooks();log('newton_muon: hooks uninstalled before EMA + eval')
...
def train_and_eval(h,device):
	...
	else:log(f"eval_only:1 skipping train_model, loading checkpoint from {h.quantized_model_path}")
